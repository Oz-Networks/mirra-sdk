...
- `headline` (string, *optional*): Optional one-line lead above the slots (≤80 chars), e.g. "Meetings are a real feature now". Skip it if the shipped lines speak for themselves. On a card with a picture this is the caption under it, so make it the takeaway.
...
`AdapterOperationResult`: Returns: card ({ cardId, authorUserId, authorName, headline, lines (each { slot, text, itemKey?, heroPageUrl?, heroPageId? } — heroPageId is the server-resolved page, so its presence is proof the picture landed), defaultBody, recipientBodies, itemKeys, artifacts, revisionCount, firstPublishedAt, lastPublishedAt }), revised (true if this revised the current burst card), priorDefaultBody (the narrative that was replaced, when revised), deprecation (present only when the legacy prose path was used — move to slots)

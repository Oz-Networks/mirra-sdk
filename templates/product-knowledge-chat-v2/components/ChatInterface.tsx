...
export default function ChatInterface({
  ...
  // Load products and resolve flow ID on mount
  useEffect(() => {
    loadProducts();
    resolveFlowId();
  }, []);
  ...
  const resolveFlowId = async () => {
    try {
      const response = await fetch('/api/sdk/v1/templates/installations/current', {
        headers: {
          'Authorization': `Bearer ${process.env.NEXT_PUBLIC_TEMPLATE_API_KEY}`,
        },
      });
      if (!response.ok) throw new Error('Failed to fetch installation');
      const data = await response.json();
      const flows = data?.flowsInstalled || data?.data?.flowsInstalled || [];
      if (flows.length > 0) {
        setFlowId(flows[0]);
      } else {
        setError('No flow configured for this template.');
      }
    } catch (err) {
      console.error('Error resolving flow ID:', err);
  ...
  const handleSubmit = async (question?: string) => {
    ...
    if (!query || !selectedProduct || isLoading || !flowId) return;
    ...
    try {
      ...
      // Execute the flow with question + context
      const result: any = await sdk.flows.execute(flowId, {
        question: query,
      ...
      const flowResult = result?.result || result?.data?.result || result || {};
      const content = flowResult.response || flowResult.content || '';
      const sources: DocumentSource[] = showSources ? (flowResult.sources || []) : [];

      setMessages(prev => prev.map(msg =>
        msg.id === assistantMessageId
          ? { ...msg, content, sources: sources.length > 0 ? sources : undefined }
          : msg
      ));
    } catch (err: any) {
        ...
        <div className="sticky bottom-0 py-4" style={{ background: 'var(--bg-primary)' }}>
          ...
          >
            <textarea
              ...
              disabled={!selectedProduct || isLoading || !flowId}
              rows={1}
            ...
            <button
              ...
              disabled={!inputValue.trim() || !selectedProduct || isLoading || !flowId}
              className="w-10 h-10 rounded-xl flex items-center justify-center transition-all flex-shrink-0"
              style={{
                background: inputValue.trim() && selectedProduct && !isLoading && flowId ? 'var(--accent-primary)' : 'var(--bg-tertiary)',
                color: inputValue.trim() && selectedProduct && !isLoading && flowId ? 'white' : 'var(--text-tertiary)',
              }}
            >
              {isLoading ? (
                <div

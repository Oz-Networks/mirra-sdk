...
Every operation is one POST. Same shape as `/mirra:messaging` — see that skill for
...
For the exact arguments of `createGroup` / `addMembers`, see `/mirra:messaging`.
...
- For anything beyond post/wait/reply (searching history, group admin, editing a message),
  reach for `/mirra:messaging` — cowork is just an orchestration layer over it.

...
export interface ItemsCreateItemArgs {
  ...
  artifacts?: any[]; // Artifact links to attach: [{ kind: "pr"|"page"|"deploy"|"doc"|"image"|"url", url, title? }]. Attach what the work produced — the PR, the published page, the deploy — so cards can preview it. Only attach links a teammate can open in a browser and see something meaningful: a page, mockup, image, PR/commit, deploy, or doc. Never API routes or endpoints, code file paths, localhost URLs, or anything that renders raw JSON. Most work has NO viewable surface — an API change, a refactor, a migration — and a PR is a viewable surface only for the developers on the team. For those, publish a short page summarizing the outcome (pages createPage) and attach it as kind "page": that page is the receipt the non-developers can open, and it is what the home card can then lead with as its picture. Never close real work with nothing attached. Always set title, in plain language a teammate recognizes at a glance ("The fix, on GitHub", "Live on production") — never commit hashes, conventional-commit prefixes, raw URLs, or timestamps.
}
...
export interface ItemsPublishUpdateArgs {
  headline?: string; // Optional one-line lead above the slots (≤80 chars), e.g. "Meetings are a real feature now". Skip it if the shipped lines speak for themselves. On a card with a hero this is the caption under the picture, so make it the takeaway.
...
function createItemsAdapter(sdk: MirraSDK) {
  return {
     ...
     * @param args.artifacts - Artifact links to attach: [{ kind: "pr"|"page"|"deploy"|"doc"|"image"|"url", url, title? }]. Attach what the work produced — the PR, the published page, the deploy — so cards can preview it. Only attach links a teammate can open in a browser and see something meaningful: a page, mockup, image, PR/commit, deploy, or doc. Never API routes or endpoints, code file paths, localhost URLs, or anything that renders raw JSON. Most work has NO viewable surface — an API change, a refactor, a migration — and a PR is a viewable surface only for the developers on the team. For those, publish a short page summarizing the outcome (pages createPage) and attach it as kind "page": that page is the receipt the non-developers can open, and it is what the home card can then lead with as its picture. Never close real work with nothing attached. Always set title, in plain language a teammate recognizes at a glance ("The fix, on GitHub", "Live on production") — never commit hashes, conventional-commit prefixes, raw URLs, or timestamps. (optional)
     ...
     * @param args.headline - Optional one-line lead above the slots (≤80 chars), e.g. "Meetings are a real feature now". Skip it if the shipped lines speak for themselves. On a card with a hero this is the caption under the picture, so make it the takeaway. (optional)
